package main

type runState string

const runStateIdle = "idle"
const runStateRunning = "running"
const runStatePaused = "paused"
const runStateFaulted = "faulted"
const runStateResetting = "resetting"

type lifecycle struct {
	state runState
}

func newLifecycle() *lifecycle {
	return &lifecycle{state: runStateIdle}
}

func (lifecycle *lifecycle) currentState() runState {
	return lifecycle.state
}

func (lifecycle *lifecycle) run() bool {
	if lifecycle.state == runStateIdle || lifecycle.state == runStatePaused {
		lifecycle.state = runStateRunning
		return true
	}
	return false
}

func (lifecycle *lifecycle) pause() bool {
	if lifecycle.state == runStateRunning {
		lifecycle.state = runStatePaused
		return true
	}
	return false
}

func (lifecycle *lifecycle) reset() bool {
	if lifecycle.state == runStatePaused || lifecycle.state == runStateFaulted {
		lifecycle.state = runStateResetting
		return true
	}
	return false
}

func (lifecycle *lifecycle) fault() bool {
	if lifecycle.state == runStateRunning || lifecycle.state == runStatePaused {
		lifecycle.state = runStateFaulted
		return true
	}
	return false
}

func (lifecycle *lifecycle) faultStart() bool {
	if lifecycle.state == runStateIdle {
		lifecycle.state = runStateFaulted
		return true
	}
	return false
}

func (lifecycle *lifecycle) completeReset() bool {
	if lifecycle.state == runStateResetting {
		lifecycle.state = runStateIdle
		return true
	}
	return false
}
