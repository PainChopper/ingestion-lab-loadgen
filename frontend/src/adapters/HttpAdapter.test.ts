...
describe('HttpAdapter', () => {
  ...
  it('starts immediately with the exact deep-frozen neutral snapshot', () => {
    ...
    expect(fetchMock.mock.calls[0]?.[0]).toBe('/api/loadgen/snapshot')
  ...
  it('rejects every command locally with exact frozen monotonic receipts', async () => {
    ...
    for (const [index, command] of commands.entries()) {
      ...
      expect(receipt).toEqual({
        ...
        error: {
          code: 'unavailable',
          message: 'Команды недоступны в HTTP snapshot mode',
          retryable: false,
          details: null,
        },
      })
    ...
    adapter.dispose()
    const afterDispose = await adapter.dispatch({ type: 'pause' })
    expect(afterDispose.commandId).toBe(`http-local-${commands.length + 1}`)
    expect(afterDispose.error?.code).toBe('unavailable')
    expect(afterDispose.snapshotRevision).toBe(0)
    expectDeepFrozen(afterDispose)
    expect(fetchMock).toHaveBeenCalledTimes(1)
  })
})
